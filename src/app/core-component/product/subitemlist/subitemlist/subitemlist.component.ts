...
import { SubItemService } from 'src/@nextdriven/Services/Sub-Item/sub-item.service';
  ...
  @Component({
    selector: 'app-subitemlist',
    templateUrl: './subitemlist.component.html',
    styleUrls: ['./subitemlist.component.scss']
  })
  ...
  export class SubitemlistComponent implements OnInit {
    ...
    ngOnInit(): void {
     ...
     getTableData(pageOption: pageSelection): void {
      ...
      this.SubItemService.GetSubItemsList(body).subscribe(
    ...
    update(row: any) {
        this.router.navigateByUrl(`/product/add-product/${row.id}`)
    ...
    this.dialog
      ...
      .subscribe((str) => {
        if (str == 'reload')
        {
          ...
          this.SubItemService.DeleteSubItem(body).subscribe(
            (response: any) => {
              if (response.isPassed == true) {
  ...
  searchData(word:string){
    ...
    let body = {
      ...
      "pageSize": 200,
      "subItemName":word   };
    this.SubItemService.SearchBySubItemName(body).subscribe(
      (response: any) => {
        if (response.isPassed == true) {
  ...
  ChangeCompanyActivity(company: any, status: boolean) {
    ...
    this.SubItemService.ChangeSubItemActivity(body).subscribe(
      (response: ResponseDto) => {
        if (response.isPassed == true) {
          this.filteredData =response.data;
          this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
