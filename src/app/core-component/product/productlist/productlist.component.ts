...
export class ProductlistComponent implements OnInit {
  columns: TableColumn<any>[] = [
    ...
    {
      label: 'Barcode No',
      property: 'barcodeNumber',
      type: 'text',
      cssClasses: ['font-medium'],
      visible: true,
    },
    {
      label: 'VaTable Status',
      property: 'isVATEnabled',
      type: 'array3',
      cssClasses: ['font-medium'],
      visible: true,
    },
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        console.log(body)
        this.ItemService.DeletedItem(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  let body = {
    ...
    "pageSize": 200,
    "itemName":word   };
  this.ItemService.SearchByItemName(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
...
ChangeCompanyActivity(company: any, status: boolean) {
  ...
  this.ItemService.ChangeItemActivity(body).subscribe(
    (response: ResponseDto) => {
      if (response.isPassed == true) {
        this.filteredData =response.data;
        this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
