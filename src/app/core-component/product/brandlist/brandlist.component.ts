...
export class BrandlistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.BrandService.GetBrandList(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/product/add-brand/${row.id}`)
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.BrandService.DeleteBrand(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  let body = {
    ...
    "pageSize": 200,
    "brandName":word   };
  this.BrandService.SearchByBrandName(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
