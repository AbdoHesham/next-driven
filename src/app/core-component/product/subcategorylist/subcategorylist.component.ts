...
export class SubcategorylistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.CategoryService.GetSubCategoriesList(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/product/sub-add-category/${row.id}`)
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.CategoryService.DeleteSubCategory(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  let body = {
    "pageIndex": 1,
    "pageSize": 200,
    "subCategoryName":word   };
  this.CategoryService.SearchBySubCategoryName(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
...
add(){
  this.router.navigateByUrl('/product/sub-add-category/')
}
}
