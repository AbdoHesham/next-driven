...
export class UserlistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.CompanyManagerService.GetUsersList(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/users/new-user/${row.userName}`)
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        console.log(body)
        this.CompanyManagerService.DeleteUser(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
ChangeCompanyActivity(company: any, status: number) {
  ...
  this.CompanyManagerService.UpdateUserActivity(body).subscribe(
    (response: ResponseDto) => {
      if (response.isPassed == true) {
        this.filteredData =response.data;
        this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
