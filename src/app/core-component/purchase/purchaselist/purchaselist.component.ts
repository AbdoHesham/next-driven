...
export class PurchaselistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.PurchaseOrderService.GetAll(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/purchase/add-purchase/${row.id}`)  
}
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.PurchaseOrderService.Delete(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  //   customerName: word,
  // };
  // this.PurchaseOrderService.(body).subscribe(
  //   (response: any) => {
  //     if (response.isPassed == true) {
