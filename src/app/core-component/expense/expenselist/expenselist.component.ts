...
export class ExpenselistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.ExpenseService.GetAll(body).subscribe(
  ...
  // }
  update(row: any) {
      this.router.navigateByUrl(`/expense/create-expense/${row.id}`)  
}
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.ExpenseService.Delete(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  this.ExpenseService.SearchByReferenceUser(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
