...
export class SupplierlistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.VendorsService.GetAll(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/people/add-supplier/${row.id}`)  
}
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.VendorsService.Delete(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  let body = {
    ...
    "pageSize": 200,
    "name":word   };
    this.VendorsService.SearchByName(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
...
ChangeCompanyActivity(company: any, status: number) {
  ...
  this.VendorsService.ChangeActivity(body).subscribe(
    (response: ResponseDto) => {
      if (response.isPassed == true) {
        this.filteredData =response.data;
        this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
