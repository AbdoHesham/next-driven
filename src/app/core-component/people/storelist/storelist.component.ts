...
export class StorelistComponent implements OnInit {
  ...
  ngOnInit(): void {
   ...
   getTableData(pageOption: pageSelection): void {
    ...
    this.StoreService.GetWarehouseList(body).subscribe(
  ...
  update(row: any) {
      this.router.navigateByUrl(`/people/add-store/${row.id}`)
  ...
  this.dialog
    ...
    .subscribe((str) => {
      if (str == 'reload')
      {
        ...
        this.StoreService.DeleteWarehouse(body).subscribe(
          (response: any) => {
            if (response.isPassed == true) {
...
searchData(word:string){
  ...
  let body = {
    ...
    "pageSize": 200,
    "warehouseName":word   };
  this.StoreService.SearchByWarehouseName(body).subscribe(
    (response: any) => {
      if (response.isPassed == true) {
